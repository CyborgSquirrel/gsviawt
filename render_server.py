...
class Service(rpyc.Service):
  ...
  def reset(self, path: str):
    ...
    # auto-distances the camera to hit the same *apparent* on-screen object
    # size (max_object_ratio) either way, but a narrower FOV needs a farther
    # camera to do that -- so left at the Blender default, our renders would
    # be ~42% too far away (tan(54.2/2)/tan(39.6/2)) for the same real
    # object, inflating absolute Z depth and giving wt a more telephoto
    # perspective than it trained on, even though the framing looks right on
    # screen. Match wt's FOV exactly instead.
